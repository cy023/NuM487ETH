...
typedef struct ARP_PACKET {
    ...
    uint16_t u16Operation;  /* operation, 1:ARP request, 2:ARP reply, 3:RARP
...
typedef struct UDP_PACKET {
    ...
    uint16_t
        u16UDPChksum; /* check sum of UDP packet, including virtual header */
} UDP_PACKET;
...
#define COMPARE_IP(ip1, ip2)                                     \
    ((ip1[3] ^ ip2[3]) | (ip1[2] ^ ip2[2]) | (ip1[1] ^ ip2[1]) | \
     (ip1[0] ^ ip2[0]))
...
#define GET32(bptr, n) \
    (bptr[n + 3] | (bptr[n + 2] << 8) | (bptr[n + 1] << 16) | (bptr[n] << 24))

#define PUT16(bptr, n, val)          \
    do {                             \
        bptr[n + 1] = val & 0xFF;    \
        bptr[n] = (val >> 8) & 0xFF; \
    } while (0)

#define PUT32(bptr, n, val)               \
    do {                                  \
        bptr[n + 3] = val & 0xFF;         \
        bptr[n + 2] = (val >> 8) & 0xFF;  \
