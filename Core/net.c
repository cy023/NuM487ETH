...
static uint8_t s_au8DhcpOptions[] = {0x63, 0x82, 0x53,         0x63,
                                     0x35, 0x01, DHCP_DISCOVER};
...
static void arp_reply(uint8_t *target_ip, uint8_t *target_mac)
{
    ...
    memcpy((char *) arp->au8SrcMac, (char *) mac_addr, 6);
    ...
    memcpy((char *) arp->au8SenderHA, (char *) mac_addr, 6);
    memcpy((char *) arp->su8SenderIP, (char *) ip_addr, 4);
...
static void udp_send(uint8_t *pu8SrcMac,
                     uint8_t *pu8SrcIP,
                     uint16_t u16SrcPort,
                     uint8_t *pu8DestMac,
                     uint8_t *pu8DestIP,
                     ...
                     uint32_t u32Len)
{
    ...
    udp->u16HdrChksum =
        ~chksum((uint16_t *) &udp->u8VerHLen, 10); /* 20 bytes */
...
int process_rx_packet(uint8_t *pu8Packet, uint32_t u32Len)
{
    ...
    if (pu8Packet[0] == 0xFF) /* this is a broadcast packet */
    {
         ...
         */
        if ((!COMPARE_IP(arp->au8TargetIP, ip_addr)) &&
            (arp->u16Type == SWAP16(PROTOCOL_ARP)) &&
    ...
    } else /* this is a multicast or unicast packet */
    {
         ...
         */
        if ((ip->u8Prot == IP_PRO_TCP) &&
            (!COMPARE_IP(ip->au8DestIP, ip_addr))) {
         ...
         */
        if ((ip->u8Prot == IP_PRO_ICMP) &&
            (!COMPARE_IP(ip->au8DestIP, ip_addr)) && (pu8Packet[34] == 0x08)) {
            IP_PACKET *tx_ip;
            ...
            memcpy((char *) tx_ip->au8SrcMac, (char *) mac_addr, 6);
            ...
            memcpy((char *) tx_ip->au8SrcIP, (char *) ip_addr, 4);
            memcpy((char *) tx_ip->au8DestIP, (char *) ip->au8SrcIP, 4);
            tx_ip->u16HdrChksum =
                ~chksum((uint16_t *) &tx_ip->u8VerHLen, 10); /* 20 bytes */
            ...
            au8TxBuf[36] = 0;
            au8TxBuf[37] = 0;
            *(uint16_t *) &au8TxBuf[36] =
                ~chksum((uint16_t *) &au8TxBuf[34], (u32Len - 34) / 2);
...
int dhcp_start(void)
{
    ...
    dhcpTx->hw_len = sizeof(mac_addr);  /* Hardware address length */
    ...
    memcpy((char *) &(dhcpTx->client_hw_addr), (char *) mac_addr, 6);
    memcpy((char *) dhcpTx->options, (char *) s_au8DhcpOptions,
           sizeof(s_au8DhcpOptions));
...
wait_offer:
    ...
    for (retry = 0; (retry < 16) && (!out); retry++) {
        ...
        udp_send(mac_addr, (uint8_t *) ip_addr, CLIENT_PORT, serverMAC,
                 serverIP, SERVER_PORT, (uint8_t *) dhcpTx,
                 DHCP_OPT_OFFSET + opt_len + 1);

        delay = 0x600000;
        while (delay--) {
            if (s_u32PktRdy) {
                NVIC_DisableIRQ(EMAC_RX_IRQn);
                memcpy((char *) &s_au8DhcpRawBuffer[1500], au8RxBuf,
                       s_u32PktRdy);
                s_u32PktRdy = 0;
                NVIC_EnableIRQ(EMAC_RX_IRQn);
...
acked:
    ...
    memcpy((char *) ip_addr, (char *) &(dhcpRx->yourIP), 4);
